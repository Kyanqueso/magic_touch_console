...
@Path("/api/v1/profiles/{profileId}/suppliers/{supplierId}/purchase-orders")
...
public class PurchaseOrderResource {
    ...
    @GET
    public PageResponse<PurchaseOrderSummaryRow> list(
    ...
    @GET
    @Path("{id}")
    public PurchaseOrderResponse get(@PathParam("profileId") long profileId,
                                     @PathParam("supplierId") long supplierId,
    ...
    public Response create(@PathParam("profileId") long profileId,
                           @PathParam("supplierId") long supplierId,
                           @Valid PurchaseOrderRequest body, @Context UriInfo uriInfo) {
        PurchaseOrderResponse created = service.create(profileId, supplierId, body);
        return Response
    ...
    @Path("{id}")
    @Consumes(MediaType.APPLICATION_JSON)
    public PurchaseOrderResponse update(@PathParam("profileId") long profileId,
                                        @PathParam("supplierId") long supplierId,
                                        @PathParam("id") long id, @Valid PurchaseOrderRequest body) {
